  ...
  transform(phoneNumber: string, formattingMode: PhoneFormat): string {
    ...
    switch (formattingMode) {
      ...
      case PhoneFormat.INTERNATIONAL: {
        const groups: string[] = [countryCode, operatorCode, firstPart, secondPart, thirdPart].map(
          (group: string[]) => group.join(''),
        );
        return '+' + groups.join(' ');
      }
      case PhoneFormat.NATIONAL: {
        const groups: string[] = [operatorCode, firstPart, secondPart, thirdPart].map(
          (group: string[]) => group.join(''),
        );
        return groups.join(' ');
      }
